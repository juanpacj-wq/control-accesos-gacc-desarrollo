// app/api/solicitud/terminar/route.ts
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { url, token } = getServerApiCredentials('TERMINAR_SOLICITUD');

    if (!url || !token) {
      console.error('Credenciales de API no configuradas para TERMINAR_SOLICITUD');
      return NextResponse.json(
    ...
    if (!response.ok) {
      return NextResponse.json(
        { 
          error: true, 
          message: data.mensaje || 'Error al terminar solicitud' 
        },
    ...
    console.error('Error en el endpoint de terminar solicitud:', error);
    return NextResponse.json(
      { 
        error: true, 
